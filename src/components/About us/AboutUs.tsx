...
const AboutUs = () => {
    ...
    return (
        <div>
            ...
            <div className="flex flex-col bg-base-100 flex-grow min-h-screen">
                <div className="flex border border-black h-1/2">
                    <img src={about} className="max-w-7xl" />
                    <div className={`text-start flex flex-col justify-center font-inter px-5 mt-20 transition-all ${animate ? 'opacity-100 transform translate-x-0' : 'opacity-0 transform translate-x-full'
                        }`}>
                        <h1 className="text-6xl font-semibold">About Us</h1>
                        <p className="text-left text-4xl max-w-md font-inter py-8">Nagai, the illustration marketplace born in Barcelona in 2023, stands as a beacon for creative artists and art enthusiasts alike. Founded with the vision of nurturing local talent, Nagai has evolved into a vibrant marketplace that connects illustrators with enthusiasts worldwide. Its creation stemmed from the need for a space where artists could not only showcase but also directly sell their works.
                        </p>
                        <Link to="/shop" className="text-3xl mb-8"><u>Shop now</u></Link>
                    </div>
                </div>
                <div className="flex font-inter flex-grow">
